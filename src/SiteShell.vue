...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('ivory.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('ivory.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('ivory.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
